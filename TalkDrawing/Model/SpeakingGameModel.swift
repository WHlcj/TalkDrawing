...
extension SpeakingGameModel {
    ...
    mutating func calculateScore(text: String, second: Int) {
        ...
        // 计算语言逻辑性评分
        var logicScore = Int.random(in: 5...10)
...
//        }
        ...
        // 最后加上语速得分整理数据
        for index in 0...3 {
            scores[index] += calculateSpeechSpeed() + 4
            if scores[index] > 10 {
                scores[index] = 10
            }
            if scores[index] < 0 {
        ...
        func calculateSpeechSpeed() -> Int {
            ...
            // 根据语速适当加分
            if durationRatio > 3 && durationRatio <= 7 {
                return 5
            }
            if durationRatio > 2 && durationRatio <= 3 {
                return 3
            }
